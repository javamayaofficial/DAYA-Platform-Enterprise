...
<div class="card shadow-sm border-0">
    <div class="card-body p-4">
        ...
        <form method="get" action="/collection/admin" class="row g-3 mb-4">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                <div class="form-check mt-2">
                    <input class="form-check-input" type="checkbox" name="include_deleted" value="1" id="includeDeletedCollection"<?= !empty($includeDeleted) ? ' checked' : '' ?>>
                    <label class="form-check-label" for="includeDeletedCollection">Deleted</label>
                </div>
            </div>
            <div class="col-12 d-flex gap-2 flex-wrap">
                <button class="btn btn-dark" type="submit">Filter</button>
                <a class="btn btn-outline-secondary" href="/collection/admin">Reset</a>
            </div>
        </form>
                ...
                <tbody>
                    <?php foreach (($result['items'] ?? []) as $item): ?>
                        <tr>
                            ...
                            <td class="text-end"><a class="btn btn-sm btn-outline-dark" href="/collection/admin/<?= e((string) ($item['id'] ?? 0)) ?>">Detail</a></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (($result['items'] ?? []) === []): ?>
                        <tr><td colspan="7" class="text-center text-secondary py-4">Belum ada collection.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        ...
        <div class="d-flex justify-content-end mt-3">
            <?php
            $pagination = $result;
            $paginationBasePath = '/collection/admin';
            $paginationQuery = [
                'status' => $status,
                'visibility' => $visibility,
